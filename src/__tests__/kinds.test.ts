...
import { classifyKind, isKind, Repost, ShortTextNote } from '../types/kinds'
...
describe('kinds', () => {
  ...
  test('kind type guard', () => {
    ...
    expect(isKind(repostedEvent, ShortTextNote)).toBe(true)
    expect(isKind(repostedEvent, Repost)).toBe(false)
  })
})
