import { NostrEvent, UnsignedEvent, VerifiedEvent } from '../core'
import { generateSecretKey, finalizeEvent, getPublicKey, verifyEvent } from '../core/pure'
import { AbstractSimplePool, SubCloser } from '../relay/abstract-pool'
...
import { SimplePool } from '../relay/pool'
import { Handlerinformation, NostrConnect } from '../types/kinds'
import type { RelayRecord } from '../relay'
import { NIP05_REGEX, getConversationKey } from '../types/nips'
      ...
      [{ kinds: [NostrConnect], authors: [bp.pubkey], '#p': [getPublicKey(this.secretKey)] }],
      {
        async onevent(event: NostrEvent) {
          ...
          try {
            ...
            o = JSON.parse(await decryptNip44(event.content, convKey))
          }
  ...
  async sendRequest(method: string, params: string[]): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const encryptedContent = encryptNip44(JSON.stringify({ id, method, params }), this.conversationKey)
