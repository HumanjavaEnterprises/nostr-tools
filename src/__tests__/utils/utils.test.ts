...
import { Queue, insertEventIntoAscendingList, insertEventIntoDescendingList, binarySearch } from '../../utils'

import type { Event } from '../../core'

describe('inserting into a desc sorted list of events', () => {
  it('insert into an empty list', async () => {
    const list0: Event[] = []
    expect(insertEventIntoDescendingList(list0, buildEvent({ id: 'abc', created_at: 10 }))).toHaveLength(1)
  })

  it('insert in the beginning of a list', async () => {
  ...
  it('insert in the beginning of a list with same created_at', async () => {
  ...
  it('insert in the middle of a list', async () => {
    const list0 = [
  ...
  it('insert in the end of a list', async () => {
    const list0 = [
      buildEvent({ created_at: 20 }),
      buildEvent({ created_at: 20 }),
      buildEvent({ created_at: 20 }),
  ...
  it('insert in the last-to-end of a list with same created_at', async () => {
    const list0: Event[] = [
      buildEvent({ created_at: 20 }),
      buildEvent({ created_at: 20 }),
      buildEvent({ created_at: 20 }),
  ...
  it('do not insert duplicates', async () => {
    const list0 = [
      buildEvent({ created_at: 20 }),
      buildEvent({ created_at: 20 }),
...
describe('enqueue a message into MessageQueue', () => {
  it('enqueue into an empty queue', () => {
    const queue = new Queue()
    queue.enqueue('node1')
    expect(queue.first!.value).toBe('node1')
  })
  it('enqueue into a non-empty queue', () => {
    const queue = new Queue()
    queue.enqueue('node1')
    queue.enqueue('node3')
    queue.enqueue('node2')
  ...
  it('dequeue from an empty queue', () => {
    const queue = new Queue()
    const item1 = queue.dequeue()
    expect(item1).toBe(null)
  })
  it('dequeue from a non-empty queue', () => {
    const queue = new Queue()
    queue.enqueue('node1')
    queue.enqueue('node3')
    queue.enqueue('node2')
