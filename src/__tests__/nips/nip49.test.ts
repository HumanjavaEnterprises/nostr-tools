...
import { decrypt, encrypt } from '../../nips/nip49'
import { hexToBytes } from '@noble/hashes/utils'

test.skip('encrypt and decrypt', () => {
  for (let i = 0; i < vectors.length; i++) {
    let [password, secret, logn, ksb, ncryptsec] = vectors[i]
    let sec = hexToBytes(secret)
    let there = encrypt(sec, password, logn, ksb)
    let back = decrypt(there as `${string}1${string}`, password)
    let again = decrypt(ncryptsec as `${string}1${string}`, password)
