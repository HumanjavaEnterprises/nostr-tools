...
import { Event, VerifiedEvent, EventTemplate } from '../core'
import { matchFilters, type Filter } from '../core/filter'
import { getHex64, getSubscriptionId } from '../utils/fakejson'
import { Queue, normalizeURL } from '../utils'
import { yieldThread } from '../utils/helpers'

export function auth(url: string, challenge: string): EventTemplate {
  return {
    kind: 22242,
...
export class AbstractRelay {
  ...
  public async auth(signAuthEvent: (evt: EventTemplate) => Promise<VerifiedEvent>): Promise<string> {
    ...
    const evt = await signAuthEvent(auth(this.url, this.challenge))
