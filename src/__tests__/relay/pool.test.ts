import { afterEach, beforeEach, expect, test } from '../helpers/test-utils'

import { SimplePool, useWebSocketImplementation } from '../../relay/pool'
import { finalizeEvent, generateSecretKey, getPublicKey, type Event } from '../../core/pure'
import { MockRelay, MockWebSocketClient } from '../helpers/test-helpers'
...
it('removing duplicates when subscribing', async () => {
...
it('same with double subs', async () => {
  let priv = generateSecretKey()
  let pub = getPublicKey(priv)
...
it('query a bunch of events and cancel on eose', async () => {
...
it('track relays when publishing', async () => {
  let event1 = finalizeEvent(
    {
      kind: 1,
