...
import { Subscription } from '../relay/abstract'
import type { Event, EventTemplate } from '../core'
import type { RelayInfo } from '../types/nips'
import { decode, NostrTypeGuard, Bech32ID, AddressPointer } from './nip19'
 ...
 */
export async function fetchRelayInformationByGroupReference(groupReference: GroupReference): Promise<RelayInfo> {
 ...
 * @param {RelayInfo} [options.relayInformation] - The relay information object.
...
export async function fetchGroupMetadataEvent({
  ...
  const groupMetadataEvent = await pool.get([normalizedRelayURL], {
    kinds: [39000],
    authors: relayInformation.pubkey ? [relayInformation.pubkey] : [],
    '#d': [groupReference.id],
  })
 ...
 * @param {RelayInfo} [options.relayInformation] - The relay information.
 ...
 * @param {RelayInfo} [options.relayInformation] - The relay information to use.
...
export function parseGroupCode(code: string): null | GroupReference {
  if (NostrTypeGuard.isNAddr(code as Bech32ID)) {
    try {
      let { data } = decode(code as Bech32ID)

      let addrData = data as AddressPointer
      let { relays, identifier } = addrData
      if (!relays || relays.length === 0) return null

      return { host: relays[0], id: identifier }
    } catch (error) {
      return null
    }
        ...
        [
          {
            kinds: [39000],
            limit: 50,
            authors: info.pubkey ? [info.pubkey] : [],
          },
        ],
        {
          onevent(event: Event) {
...
/**
 * Fetches relay information.
 *
 * @param url - The URL of the relay.
 * @returns A promise that resolves to the relay information.
 */
export async function fetchRelayInformation(url: string): Promise<RelayInfo> {
  const response = await fetch(`${url}/.well-known/nostr.json`)
  if (!response.ok) {
    throw new Error(`Failed to fetch relay information: ${response.statusText}`)
  }
